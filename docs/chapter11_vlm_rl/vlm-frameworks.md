...
VLM-RL 在机器人领域的应用前景最为广阔。机器人需要从摄像头输入中理解环境，然后做出操作决策。和下一章讨论的连续控制不同，VLM-RL 的核心是"用视觉理解来指导动作"——不是直接从像素到力矩，而是先理解"面前是什么物体"，再决定"该怎么操作"。
...
**仿真精调**阶段，针对目标机器人的具体场景做精细化训练。这一步会引入域随机化（Domain Randomization，回顾第 11 章）——随机化光照、纹理、物体位置等参数，让策略在各种条件下都能工作。
...
## 10.3.5 从文本 RL 到多模态 RL：回顾与展望

回顾第 3 章到第 10 章的完整学习路径，我们看到 RL 从最简单的表格方法一路发展到了多模态的复杂场景：
...
| 第 11 章：连续控制 | 状态向量      | 连续（力矩）  | 环境内置  | SAC/TD3   |
| 第 10 章：VLM RL   | 图像 + Token  | 离散（token） | 规则+模型 | GRPO      |
...
把本章的 VLM RL 和[第 9 章](../chapter12_agentic_rl/intro)的 Agent RL 结合，会面临三个额外的挑战：
...
**3. 跨模态 Credit Assignment。** 在一条 10 轮的轨迹中，第 2 轮的视觉理解错误可能导致第 5 轮的工具调用失败。这比纯文本 Agent 的 credit assignment 更难，因为跨模态的错误传播链更长、更隐晦。[第 9 章](../chapter12_agentic_rl/multi-turn-rl)讨论的 ORM vs PRM 取舍在这里更加突出。
...
2. **再练工具调用**：用[第 9 章的工具调用 RL](../chapter12_agentic_rl/tool-use-agents)训练基本的工具使用模式
