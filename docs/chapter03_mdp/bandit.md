...
至此，你应该抓住了这节的主线：**策略就是智能体选择动作的规则**。在两台老虎机里，策略可以很简单：

- 均匀随机：不管观察到什么，都 50/50 选。
- 始终选 A：不探索，直接利用已知最优动作。
- 先试后定：先用样本估计哪个动作好，再把剩余机会用在看起来更好的动作上。

这三种策略面对的是同一个环境，但长期回报完全不同。原因不是机器变了，而是**选择动作的规则变了**。

本节真正要带走的是三点：

1. **期望回报**用来衡量一个策略长期好不好。
2. **探索**用来收集信息，但样本少会误判。
3. **利用**用来把已知信息变成回报，但如果前面的估计错了，利用也会跟着错。

所以 RL 的核心不是"算出 A 比 B 好"这么简单，而是设计一套策略：在不知道答案的情况下，什么时候该继续试，什么时候该相信已有信息并开始利用。

老虎机只有一个简化点：它没有状态变化。真实问题里，智能体看到的局面会变，策略也不再只是"选 A 还是选 B"，而是"在当前状态下选什么动作"。下一节会把这些概念形式化成 MDP：[MDP 五元组、折扣回报与策略](./mdp)
